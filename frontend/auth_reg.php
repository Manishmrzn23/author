 <?php
        ...
        $secure_pass = password_hash($pass, PASSWORD_DEFAULT);
          ...
          $sql = "INSERT INTO auth_reg (`auth_id`,`name`, `email`,`password`, `photo`,`facebook`, `twitter`,`instagram`) VALUES ('1','$name', '$email','$secure_pass','$filename','$facebook','$twitter','$instagram')";
          $data = $_POST;
         ...
         elseif ($data['password'] !== $data['cpassword']) {
          ...
          $select = mysqli_query($con, "select * from auth_reg where email = '".$_POST['email']."'");
          if(mysqli_num_rows($select)>0) {
              $message=alert_message(" alert-danger","This email already exists");
          }
          elseif(mysqli_query($con, $sql)){
          $message=alert_message(" alert-success","Data uploaded successfully. ".'<a href="Login.php">login</a>');
          }
          else{
